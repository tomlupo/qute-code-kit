...
**Known Limitations:**
- **No date filtering**: API ignores date range parameters (`?from=`, `?dateFrom=`, `?period=`)
- **Always full history**: Returns complete time series regardless of query parameters
- **No pagination**: Single response contains all data points

Tested parameters that do NOT filter data: `?from=`, `?dateFrom=`, `?start_date=`, `?period=`. All return identical full history.
