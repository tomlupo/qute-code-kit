...
| **Security guards** (toggleable via `/guard`) | destructive (blocks `rm -rf /`, `git reset --hard`, etc.), git-workflow (best-effort agent-side speed bump against direct commit/push on a repo's protected + integration branches — parser-based with known gaps, `pre-push` is the enforcement layer; opt in per repo with `.claude/git-guard.json`), secrets (blocks writes containing API keys / private keys / tracked `.env`), audit (pip-audit on dependency changes), lakera (prompt-injection screening on untrusted tool output), langfuse (every-tool-call observability), provenance (auto-injects the `[agent:]`/`[session:]` identity tag on automated Linear-MCP / `gh pr` writes) |
