...
# The review/merge queue. status.sh otherwise only surfaces MERGED-PR
# worktrees (cleanup candidates) — it never showed what's still OPEN, so an
# open PR awaiting review sat invisible until you ran `gh pr list` by hand.
# This is the "frontier" of in-flight review work for this repo.
if command -v gh >/dev/null 2>&1; then
  open_pr_lines=()
  while IFS= read -r pr_line; do
    [ -n "$pr_line" ] && open_pr_lines+=("  $pr_line")
             ...
             2>/dev/null)
  if [ "${#open_pr_lines[@]}" -gt 0 ]; then
    echo "Open PRs (awaiting review/merge):"
    printf '%s\n' "${open_pr_lines[@]}"
    echo
  fi
fi
