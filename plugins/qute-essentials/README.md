...
**A repo carrying `.claude/hooks/git-workflow-guard.py` is not opted in twice — it is running a stale fork.** Three repos hand-copied this guard on 2026-06-18, before the fix for resolving which repo a `git -C <path>` command targets (defect 65 above and its neighbours), so the copy can raise a FALSE BLOCK the plugin's guard would not. `scripts/migrate_git_guard.py --repo .` removes that file, surgically unwires it from `.claude/settings.json` / `settings.local.json` (only the entries naming it; sibling structure and order always survive, and their bytes too for canonically formatted JSON, which is re-emitted at its own detected indent; emptied containers are pruned), and stamps a minimal `.claude/git-guard.json` when none exists — never rewriting one the repo already wrote. `--check` reports the plan without writing; re-running is a no-op. `/setup-qute-repo` step 9 runs it before the `pre-push` installer.
