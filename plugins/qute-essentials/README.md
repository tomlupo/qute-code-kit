...
Here-doc bodies (`cat <<EOF … EOF`) are skipped as data, not parsed as commands; `<<<` is a here-string and is left alone. Redirections are stripped wherever they appear — before the command word (`>/tmp/out git commit`) or after it (`git push origin >main`, where `main` is a *filename*) — and `2>&1` / `>|file` are read as redirection syntax rather than as a background `&` or a pipe. The segment scan is quote-aware, so a `)` or a `;` inside `git commit -m "done)"` is text, not syntax. Shell reserved words that stand in front of a command are peeled before parsing it, so `if …; then git commit; fi`, `while …; do git push origin main; done`, `! git …` and `time git …` are read as the git commands they contain.
...
Defects review has found, **all now handled** — kept as evidence the tail is real, not as a checklist that's complete. 1–9, 14, 15, 19–21, 23, 24 and 27–30 are parsing; 10–13, 16–18, 22, 25 and 26 are the environment axis, and are why that axis is written down at all. Most let something *through*; 24, 27, 29 and 30 **blocked** something they shouldn't have, which is a defect on the same footing — a guard that cries wolf gets turned off:
