...
**Dry runs are never blocked.** `git commit --dry-run` and `git push --dry-run`/`-n` write nothing. Note `git commit -n` is `--no-verify` — a *real* commit — and stays blocked; option values are skipped, including inside short-option clusters, so `git commit -m --dry-run` and `git commit -am --dry-run` are real commits too (while `-amwip --dry-run`, which carries its message in the cluster, is a genuine dry run).
