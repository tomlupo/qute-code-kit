...
**Dry runs are never blocked.** `git commit --dry-run` and `git push --dry-run`/`-n` write nothing, including clustered (`git push -vn`). It's the *last* flag that counts, so a `--no-dry-run` after one is a real write. Note `git commit -n` is `--no-verify` — a *real* commit — and stays blocked; option values are skipped, including inside short-option clusters, so `git commit -m --dry-run` and `git commit -am --dry-run` are real commits too (while `-amwip --dry-run`, which carries its message in the cluster, is a genuine dry run).
...
Defects review has found, **all now handled** — kept as evidence the tail is real, not as a checklist that's complete. 1–9, 14, 15, 19–21, 23, 24, 27–32, 34–36, 38–41, 45–48 and 51 are parsing; 10–13, 16–18, 22, 25, 26, 33, 37, 42–44, 49 and 50 are the environment axis, and are why that axis is written down at all. Most let something *through*; 24, 27, 29–32, 36, 42 and 43 **blocked** something they shouldn't have, which is a defect on the same footing — a guard that cries wolf gets turned off:
