...
```bash
bash "$HOME/bin/qute_coder_flow.sh" $@
```
...
- `~/bin/qute_coder_flow.sh` — the chain orchestrator this skill dispatches to.
- `~/bin/qute_coder_pr.sh` — the open-only kernel (step 1).
