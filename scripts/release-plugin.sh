...
#
# Bumps `plugins/<plugin>/plugin.json::version`, regenerates
...
Bumps plugins/<plugin>/plugin.json, regenerates marketplace.json, writes
a CHANGELOG entry from Conventional Commits since the last tag, commits,
and tags. Caller pushes.
EOF
  exit 2
}
...
# Group conventional commits since last tag into BREAKING / Feat / Fix / Other.
...
if [ -z "$breaking$feats$fixes" ] && [ -n "$last_tag" ]; then
  echo "warning: no feat/fix/breaking commits since $last_tag — releasing anyway." >&2
fi
