...
docs/datasets/{datasetname}.md      # Documentation (use template below)
```
...
4. **Document**: Create/update `docs/datasets/{datasetname}.md` (use template below)
...
Every dataset needs a `docs/datasets/{datasetname}.md` file. Use `/pipeline-docs` skill for the full template. At minimum, document:

- **Overview**: Purpose, source, update frequency
- **Schema**: Column names, types, descriptions, examples
- **File locations**: Raw, intermediate, processed paths
- **Loading**: Working code snippet to load the data
- **Data quality**: Known issues, validation status, completeness
