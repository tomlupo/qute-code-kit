...
try:
    ...
    for env_path in [
        ...
        / ".env",  # Skill root (.claude/skills/market-datasets/.env)
        ...
        try:
            if format == "csv":
                return pd.read_csv(
                    cache_path,
                    parse_dates=["Date"]
                    if "Date" in pd.read_csv(cache_path, nrows=0).columns
                    else None,
                )
            elif format == "json":
...
if __name__ == "__main__":
    ...
    assert (
        create_identifier("PKO", "20240101", "20241231") == "pko_d_20240101_20241231"
    ), "Identifier creation failed"
